...
*Draft updated: 2026-04-06 (Session 28). Status: All 7 IEEE submission blockers resolved — abstract trimmed to ≤150 words; I.E corrected to 5-section list (II–VI); III.B Matsubara ξ_n² weight factor added explicitly; V.C [refs needed] replaced with [16]; IV.E stray bullets converted to prose table note; footer updated to Session 28; finite-slab correction (1.2% for QNT-26-100) added in V.C. Physics engine bugs fixed: p_max clamped to 1e6 in all 9 outer-integrand functions (src/lifshitz.py + casimir_tools/_core.py); HBAR/(4π²c²) prefactor confirmed correct; 2-osc `points` list sorted. Remaining user actions: author name / email / ORCID; cover letter personal details. PyPI: casimir-tools v0.1.3 published.*
