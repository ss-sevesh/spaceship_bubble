...
**Computational efficiency note.** For computational efficiency, the NSGA-II inner loop uses the non-retarded Hamaker approximation E_fast = −A/(12πd²)·(1 − χκ²) rather than the full Lifshitz double integral. This approximation overestimates |E_Casimir| by a factor of 2× at d=10 nm and up to 14× at d=100 nm relative to the full Lifshitz result; it is used solely to accelerate the 5000-evaluation optimization loop. All energies reported in the results section use the full Lifshitz integral via `casimir_energy_chiral_asymmetric()` (Silveirinha 2010), evaluated in the post-optimization validation pass on the final Pareto front. The fast-model objective function correctly ranks designs by stiction severity (sign and ordering are preserved) but its absolute energy scale is not physically meaningful.
...
- Td phase (ε_⊥ = 18.60, ε_∥ = 8.80) produces ~1.6× stronger Casimir vs hex phase (ε_∥ = 1.56) at d = 1–50 nm (ratio 1.64 at d=1 nm, 1.36 at d=53 nm; see Table casimir_td_wte2.png)
...
The WTe₂ counter-plate retains its value as a *passive* stiction suppressor: the separation-dependent TM suppression from anisotropy (Sec. IV.B, ~10% at d=10 nm) is independent of chirality and remains valid. The separation-dependent Te|WTe₂ stiction reduction from anisotropy alone is ~10% at d=10 nm and ~5% at d≥50 nm; replacing WTe₂ with a second Te plate removes the passive TM suppression but enables full chirality-driven repulsion.
...
The hex-WTe₂ counter-plate retains independent engineering value as a passive stiction suppressor: ε_zz = 1.56 ≈ vacuum suppresses the TM Casimir channel by ~10% at d=10 nm and ~5% at d≥50 nm, independent of chirality and applicable to any MEMS design using a WTe₂ substrate. The Td-WTe₂ Weyl phase (HSE06-level estimate: ε_⊥ = 18.60, ε_∥ = 8.80) produces ~1.6× stronger baseline coupling than the hex phase (ratio 1.64 at d=1 nm, 1.36 at d=53 nm) but provides no passive TM suppression.
...
| 2 | casimir_aniso.png | Anisotropic vs isotropic Lifshitz: WTe₂ ε_∥=1.56 suppresses TM contribution by ~10% at d=10 nm and ~5% at d≥50 nm (separation-dependent) |
...
| 9 | casimir_td_wte2.png | Td-WTe₂ (Weyl) vs hex-WTe₂ vs symmetric Td\|Td: ~1.6× stronger TM coupling in Weyl phase (ratio 1.64 at d=1 nm, 1.36 at d=53 nm) |
