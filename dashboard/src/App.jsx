...
const App = () => {
  ...
  const handleDownloadReport = async () => {
    ...
    const lines = [
      ...
      `  ε_eff (EMA)         : ${selectedDesign.eps_eff?.toFixed(3)}  (Maxwell-Garnett composite)`,
      '',
      ...
      `  Thermal fraction    : f_T = ${obj.thermal_fraction?.toExponential(3)}  (E_classical/|E_quantum|)`,
      `  Slab correction     : ${((1 - (obj.slab_thickness_correction ?? 1)) * 100)?.toFixed(2)}%  (Airy transfer-matrix deviation from thin-film limit)`,
      '',
      '── Physical Constants (SI) ──────────────────────────────',
      '  ℏ = 1.0545718×10⁻³⁴ J·s',
      '  c = 2.99792458×10⁸ m/s',
      '  k_B = 1.380649×10⁻²³ J/K  (T = 300 K run)',
      '',
      '── Materials ────────────────────────────────────────────',
      '  Plate 1 : Tellurium (Te, mp-19, P3₁21)',
      '            ε_eff = 164.27  (isotropic Lifshitz),  n = 10.88',
      '            Uniaxial: ε_⊥ = 130.86   ε_∥ = 231.09',
      '  Plate 2 : WTe₂ (mp-1023926, P-6m2)',
      '            ε_eff = 6.16  (isotropic Lifshitz)',
      '            Uniaxial: ε_⊥ = 8.46   ε_∥ = 1.56',
      '  Td-WTe₂ variant (DFT-HSE06):',
      '            ε_eff = 15.33  (isotropic),  ε_⊥ = 18.60   ε_∥ = 8.80',
      '',
      '── References ───────────────────────────────────────────',
